...
SimonK_I2C_ESC motor_1(i2c, ESC_ADDRESS_1,6);
SimonK_I2C_ESC motor_2(i2c, ESC_ADDRESS_2,6);
...
void updateMotors(){
        ...
        if(motor_1.isAlive()){
            ...
            pc.printf("%d",motor_1.rpm());pc.printf(" Actual Motor1 RPM\t\t");
            float throttle_compensation = motor_1_pid.compute();
            pc.printf("%f",throttle_compensation);pc.printf(" Throttle Compensation Motor1\t\t");
            motor_1.set((short)throttle_compensation);
        }
        if(motor_2.isAlive()){
            motor_2_pid.setProcessValue(motor_2.rpm());
            pc.printf("%d",motor_2.rpm());pc.printf(" Actual Motor2 RPM\t\t");
            float throttle_compensation_2 = motor_2_pid.compute();
            pc.printf("%f",throttle_compensation_2);pc.printf(" Throttle Compensation Motor2\t\t");
            pc.printf("\n\r");
